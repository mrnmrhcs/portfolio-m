<?php include $_SERVER['DOCUMENT_ROOT'] . '/snippets/document/header/index.php'; ?>

<main class="prjct prjct-behance content">
  <div class="prjct-header">
    <div class="container">
      <div class="row">
        <div class="col-12">
          <h1>Behance <br /> Case</h1>
          <div class="prjct-img-hero prjct-img-full">
            <img src="../assets/images/biskitty-img-1160x2700.png" alt="">
          </div>
        </div>
      </div>
    </div>
  </div>
  <div class="prjct-content">
    <div class="container">
      <div class="row">
        <div class="col-12">
          <div class="prjct-img-1 prjct-img-full">
            <img src="../assets/images/ruffcycles-hero-1160x5960.png" alt="">
          </div>
        </div>
      </div>
    </div>
  </div>
</main>

<?php include $_SERVER['DOCUMENT_ROOT'] . '/snippets/document/footer/index.php'; ?>
